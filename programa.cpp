...
// 2. Función de registrar materias / Visualización de las materias.
void registrarMaterias(Estudiante &estudiante)
{
    string continuar;
...
int crearTarea(Estudiante &estudiante, Tarea tareas[], Cursos materias[], int contadorTarea)
{
    ...
    { // impresion de nombre y codigo de materia
    ...
    do
    { // Validación del ingreso del codigo del curso en la tarea
        int numTarea = 0;
    ...
    {
        cout << endl
             << "Nombre de la tarea " << i + 1 << ": " << tareas[i].nombre << endl;
...
// 5. Función para modificar una tarea específica.
void modificarTarea(Tarea tareas[], int contadorTarea)
{
    system("ClS");
...
// 6. Función para buscar una tarea específica a partir de una fecha de entrega.
void buscarFechaTarea(Tarea tareas[], int contadorTarea)
{
    string repeticion = "s";
    ...
    while (repeticion == "s")
    {
        ...
        for (i = 0; i < contadorTarea; i++)
        {
            if (tareas[i].dia == diaTarea)
            {
                ...
                if (tareas[i].mes == mesTarea)
                {
                    ...
                    {
                        cout << endl
                             << "Tareas que coinciden con la busqueda. \n";
                        cout << endl
                             << "Nombre de la tarea " << i + 1 << ": " << tareas[i].nombre << endl;
                        cout << "Descripcion: " << tareas[i].descripcion << endl;
                        cout << "Estado: " << tareas[i].estado << endl;
        ...
        {
            cout << endl
                 << "Ninguna tarea coincide con el criterio de búsqueda.\n";
        }
...
// 7. Función para cambiar el estado de una tarea Específica.
void cambiarEstadoTarea(Tarea tareas[], int contadorTarea)
{
...
// 8. Función para eliminar una tarea Específica.
int eliminarTarea(Tarea tarea[], Tarea TareaEliminada[], int contadorTarea, int contadorEliminado)
{
    int numTarea, condicionEliminar;
    system("CLS");
    if (contadorTarea == 0)
    { // Condicion si no existen tareas para borrar
        cout << "No tiene ninguna tarea actualmente\n ";
        system("PAUSE");
        condicionEliminar = 0;
    }
    else
    { // Condición si existen tareas para borrar
        cout << "Las tareas creadas anteriormente son: " << endl
             << endl;
        for (int i = 0; i < contadorTarea; i++)
        { // Impresión del número de la tarea con la tarea. Ejemplo: Tarea 1: cartelera
            if (tarea[i].nombre != "")
            {
        ...
        do
        { // Validación en caso de que el usuario un número de una tarea que no exista o ingrese letras
            cin >> numTarea;
            if (cin.fail())
            {
                cin.clear();
        ...
        cout << endl
             << "       Tarea " << numTarea + 1 << ": " << tarea[numTarea].nombre << endl;
        ...
        for (int i = numTarea; i < contadorTarea; i++)
        { // for para que desde la tarea que se elimine todas las tareas se corran un puesto para atras
...
// 9. Función para visualizar las tareas de una materia específica.
void historialTareas(Tarea tarea[], Tarea TareaEliminada[], int contadorTarea, int contadorEliminado, Estudiante &estudiante)
{
    ...
    { // Condición si no existe ninguna tarea, ni ninguna tarea eliminada
        cout << "No ha ingresado ninguna tarea. Por lo tanto no tiene ningún historial.\n\n";
        system("PAUSE");
    }
    else if (contadorTarea == 0 && contadorEliminado != 0)
    { // Condición si no existe ninguna tarea, pero si has eliminado tareas
        ...
        { // impresión de nombre y código de materia
            cout << setw(20) << left << estudiante.materias[i].nombre << setw(10) << left << estudiante.materias[i].codigo << endl;
        }
        ...
        do
        {
            ...
            for (int i = 0; i < estudiante.numMateriasInscritas; i++)
            {
                cout << estudiante.materias[i].codigo << endl;
            ...
            if ( variable ) {
                cout << endl
                << "El código que ingreso no pertenece a ninguna materia." << endl
                << "Ingrese nuevamente el código de la materia: ";
            }
        } while (variable);
        ...
        for (int i = 0; i < contadorEliminado; i++)
        {
            ...
            { // Condición en caso de que el buscador sea igual al código de curso que le asignamos al crear la tarea
                numTarea++;
    ...
    { // El código de esta condición tiene una lógica muy similar a la de la condición anterior

        cout << "Las materias con su código respectivamente son: " << endl
             << endl;
        cout << left << setw(20) << "Materia" << setw(10) << "Código" << endl;
        for (int i = 0; i < estudiante.numMateriasInscritas; i++)
        { // impresión de nombre y código de materia
        ...
        for (int i = 0; i < estudiante.numMateriasInscritas; i++)
        {
            if (buscador == estudiante.materias[i].codigo)
            {
                cout << "El historial total de tareas en la materia " << estudiante.materias[i].nombre << " es: " << endl
                     << endl
                     << endl;
            }
        }
...
bool validarcorreo(string correo)
{
    ...
    if (condicion == 0)
    {
        for (int i = 0; i < cantletras; i++)
        {
            ...
            {
                if (i == 0)
                { // Arroba no puede ser inicio del correo
                    break;
                }
                else
                {
                    for (int j = i; j < cantletras; j++)
                    { // El for empieza a contar desde la posicion del arroba hasta encontrar un punto
                        if (correo[j] == punto[0])
                        {

                            if (j - i <= 1)
                            { // El arroba y el punto no pueden ir seguidos
                                break;
                            }
                            else
                            {
